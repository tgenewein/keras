...
def test_top_k_sparse_categorical_accuracy():
    ...
    success_result = K.eval(metrics.top_k_sparse_categorical_accuracy(y_true,
			    y_pred, k=3))
    assert success_result == 1
    partial_result = K.eval(metrics.top_k_sparse_categorical_accuracy(y_true,
			    y_pred, k=2))
    assert partial_result == 0.5
    failure_result = K.eval(metrics.top_k_sparse_categorical_accuracy(y_true,
			    y_pred, k=1))
    assert failure_result == 0   
